# 3. Python package manager for Conda-Forge

**Why an additional Python package manager?**

To install certain binary package dependencies, `pip` or "pip-only" package managers (like `uv` and other) **we highly recommend using**
a Python environment management and package installation system based on [Conda-Forge](https://conda-forge.org/). 
...
Especially for Windows computers where installing packages like `GDAL` becomes difficult (WSL2 would be Linux again), or when experimenting even on Linux and MacOSX, `conda` packages allow for a lot of quality of life improvements.
...
- [Pixi.sh](https://pixi.sh/latest/) Pixi is a fast, modern, and reproducible package management tool for developers of all backgrounds, that basically combines the speed and ergonomy of [uv](https://docs.astral.sh/uv/) with the binary package access to conda-forge.

::: {.callout-important}
In this workshop, we will use Micromamba to install our Python environment!
...
From here you can use micromamba for
installing additional packages or creating environments with an
environment.yml file.

The next step is [4. Creating environments and install
